...
import { planVariations, type BrandMode, type SignSizeHint } from "@/lib/ai/variation-planner"
...
import type { GenerationResult, Candidate, ReferenceStyle, Placement, VariationCount, TextStyling } from "@/types/flow"
...
// Allow up to 60 seconds — Netlify Pro supports this via @netlify/plugin-nextjs
...
export async function POST(req: NextRequest) {
  try {
    ...
    const signSizeRaw = formData.get("signSize") as string | null
    ...
    const placement: Placement = placementRaw
      ...
      : { centerX: 0.5, centerY: 0.22, width: 0.68, height: 0.14, rotation: 0, facadeConfidence: 0.85 }
    ...
    const signSize: SignSizeHint | null = signSizeRaw ? JSON.parse(signSizeRaw) : null
    ...
    const guidedStorefrontBuffer = await applyBrushGuideToStorefront(storefrontBuffer, placementBrushBuffer)
    ...
    if (brandAssetFile) brandAssetImageData = await fileToImageData(brandAssetFile)
    ...
    const specs = await planVariations(references, variationCount, resolvedBrandText, brandMode, textStyling, signSize)
    ...
    const generationPromises = specs.map(async (spec, i) => {
      ...
      if (isGeminiProvider(provider) || provider === "fal") {
        ...
        slotLines.push(`Image ${slot++}: storefront — gold/yellow shows zones where new signage goes; edit only those areas.`)
      ...
      const params: GenerateImageParams = {
        prompt: fullPrompt,
        provider,
        negativePrompt: "blurry, low quality, text errors, spelling mistakes, distorted letters, unrealistic, cartoon",
        placement,
    ...
    const response: GenerationResult = {
      jobId: randomUUID(),
      candidates,
      compatibility: { allowedLightModes: allAllowedModes },
    }
...
async function applyBrushGuideToStorefront(imageBuffer: Buffer, brushPngBuffer: Buffer): Promise<Buffer> {
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      const i = (y * W + x) * 4
      if (v > threshold) {
        overlay[i] = 255; overlay[i + 1] = 215; overlay[i + 2] = 64; overlay[i + 3] = 130
      }
    }
  }

  const overlayPng = await sharp(overlay, { raw: { width: W, height: H, channels: 4 } }).png().toBuffer()
  return sharp(imageBuffer).composite([{ input: overlayPng, blend: "over" }]).jpeg({ quality: 92 }).toBuffer()
}
...
async function compositeSignOntoOriginalWithBrush(
  ...
  const origRgb = await sharp(originalBuf).resize(origW, origH).removeAlpha().raw().toBuffer()
  const genRgb = await sharp(generatedBuf).resize(origW, origH, { fit: "fill" }).removeAlpha().raw().toBuffer()
  const blendWeight = await sharp(brushPngBuffer).resize(origW, origH, { fit: "fill" }).greyscale().raw().toBuffer()
  ...
  const jpeg = await sharp(out, { raw: { width: origW, height: origH, channels: 3 } })
    .jpeg({ quality: 92 })
    .toBuffer()
