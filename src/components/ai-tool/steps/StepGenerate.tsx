...
export function StepGenerate() {
  const {
    storefrontFile, storefrontPreviewUrl, brandAssetFile, brandText, textStyling,
    selectedReferences, placement, placementBrushFile, variationCount,
    setSelectedProvider, setGenerationResult, setLeadId, leadId,
    signSize, doorDetection, source, goNext,
  } = useFlowStore()
  ...
  useEffect(() => { setSelectedProvider("gemini-2.5") }, [setSelectedProvider])
  ...
  const buildPromptPreview = () => {
    ...
    try {
      ...
      const brandMode = brandAssetFile && brandText ? "logo-and-text" : brandAssetFile ? "logo-only" : "text-only"
      const isAwning = reference.id === "awning"
      const isLightBox = reference.id === "light-box"
      const contentDesc = brandMode === "logo-only" ? "the logo from Image 2"
        : brandMode === "text-only" ? `the text '${resolvedBrandText}'`
      ...
      const colorDesc = brandMode === "logo-only" ? "The colors must match exactly the logo provided in Image 2."
        : textStyling?.color ? `The letters should be ${textStyling.color}.`
      ...
      let prompt = ""
      if (isAwning) {
        let awningColorDesc = brandMode === "text-only" ? `AWNING COLOR: ${textStyling?.color || "a professional color"}. TEXT COLOR: White or cream.`
          : brandMode === "logo-only" ? "AWNING COLOR: Matches/complements the logo colors. LOGO: Exact original colors."
          : "AWNING COLOR: Matches/complements the logo. LOGO: Original colors. TEXT: White/cream."
        prompt = `Generate a photorealistic architectural photo of the storefront. Inside the area marked by the yellow highlight, place a new, professional fabric awning that clearly displays ${contentDesc}. The awning must be made of heavyweight canvas fabric with natural draping and visible texture. The graphics should appear screen-printed or vinyl-applied onto the fabric surface. ${awningColorDesc} The awning must be properly anchored to the building with a visible metal frame—do not let it float. Use natural daylight only—no artificial glow. Completely replace the yellow highlighted area with this new awning.`
      } else if (isLightBox) {
        prompt = `Generate a photorealistic architectural photo of the storefront. Inside the area marked by the yellow highlight, place a new, high-end illuminated light box sign that clearly displays ${contentDesc}. The sign must be a sleek aluminum cabinet with a translucent acrylic face panel. ${colorDesc} Internal LED illumination creates an even, soft glow. Ensure the sign is physically mounted to the wall with visible brackets—do not let it float. Completely replace the yellow highlighted area with this new signage.`
      } else {
        prompt = `Generate a photorealistic architectural photo of the storefront. Inside the area marked by the yellow highlight, place a new, high-end ${signType} sign that clearly reads ${contentDesc}. The sign must be dimensional, 3D channel letters${fontDesc}. ${colorDesc} The sign must have ${lightDesc}. Ensure the new signage is physically mounted to the wall with visible brackets—do not let it float. The text must be sharp, legible, and integrated into the building's facade. Completely replace the yellow highlighted area with this new signage.`
      }

      const sizeInfo = signSize?.widthIn && signSize?.heightIn
        ? `\nEstimated sign area: ${signSize.widthIn}" × ${signSize.heightIn}" (${signSize.method})`
        : ""
      ...
      setPreviewPrompt(prompt + sizeInfo + imagesInfo)
    } catch { setPreviewPrompt("Could not build preview") }
  ...
  const handleTogglePromptPreview = () => {
    ...
    if (newState && !previewPrompt) buildPromptPreview()
  }

  const handleGenerate = async (capturedLeadId?: string | null) => {
    if (!storefrontFile || !placement || !placementBrushFile || !variationCount || selectedReferences.length !== 1) return
    ...
    try {
      ...
      if (signSize) formData.append("signSize", JSON.stringify(signSize))
  ...
  const submitSilently = async (): Promise<string | null> => {
    try {
      ...
      fd.append("name", lead.name ?? ""); fd.append("email", lead.email ?? "")
      fd.append("phone", lead.phone ?? ""); fd.append("company", lead.company ?? "")
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500 mt-1">Review your setup and generate your sign mockup.</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl divide-y divide-gray-100">
        ...
        <SummaryRow label="Style references" value={selectedReferences.map((r) => r.name).join(", ") || "—"} />
        <SummaryRow label="Variations" value={variationCount ? `${variationCount} design${variationCount > 1 ? "s" : ""}` : "—"} />
      </div>
      ...
      <div className="border border-gray-200 rounded-xl overflow-hidden">
        ...
        >
          ...
          {showPromptPreview ? <ChevronUp size={16} className="text-gray-400" /> : <ChevronDown size={16} className="text-gray-400" />}
        </button>
        {showPromptPreview && (
          <div className="p-4 bg-white border-t border-gray-100">
            {loadingPreview ? (
              ...
              <p className="text-sm text-gray-500 text-center py-4">Click to load prompt preview</p>
            )}
          </div>
        )}
      </div>
      ...
      {status === "error" && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4">
          ...
          <button type="button" onClick={() => setStatus("idle")} className="text-xs text-red-500 underline mt-1">Try again</button>
        </div>
      )}
      ...
      <button
        ...
        className={cn(
          ...
          isLoading || status === "done" ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-primary text-primary-foreground hover:bg-primary/90"
        )}
      >
        {isLoading ? (
          <><Loader2 size={16} className="animate-spin" />{status === "uploading" ? "Uploading…" : "Generating…"}</>
        ) : (
          <><Sparkles size={16} />Generate My Sign{variationCount && variationCount > 1 ? "s" : ""}</>
        )}
      </button>
      ...
      {showLeadModal && (
        <LeadCaptureModal
          ...
          companyName={brandText ?? undefined}
...
function SummaryRow({ label, value, imageUrl }: { label: string; value: string; imageUrl?: string }) {
  return (
    <div className="flex items-center gap-3 px-4 py-3">
      <span className="text-xs font-medium text-gray-400 w-28 shrink-0">{label}</span>
      <div className="flex items-center gap-2 min-w-0">
        {imageUrl && <img src={imageUrl} alt="" className="w-8 h-8 object-cover rounded" />}
