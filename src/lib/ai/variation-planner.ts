...
import type { ReferenceStyle, VariantSpec, VariationCount, FontStyle, TextStyling } from "@/types/flow"
...
export interface SignSizeHint { widthIn: number; heightIn: number; method: string }
...
export async function planVariations(
  ...
  signSize?: SignSizeHint | null,
): Promise<VariantSpec[]> {
  const primary = references[0]

  // Always use the local deterministic planner to preserve Gemini quota for image generation.
  return planDeterministic(primary, variationCount, brandText, brandMode, textStyling, signSize)
}
  ...
  for (let i = 0; i < count; i++) {
    ...
    specs.push({
      ...
      prompt: buildPrompt({
        ...
        lightMode,
        brandMode,
        textStyling: effectiveTextStyling,
        signSize,
      }),
    })
  }
...
function buildPrompt({
  ...
  const sizeDesc = signSize && signSize.widthIn > 0 && signSize.heightIn > 0
    ? `The sign area is approximately ${signSize.widthIn}" wide × ${signSize.heightIn}" tall — design the sign to fill this space proportionally.`
    : ""
