...
export function StepUpload() {
  ...
  const detectDoorSilently = useCallback(
    async (previewUrl: string) => {
      try {
        const res = await fetch("/api/detect-door", {
          method: "POST",
        ...
        setDoorDetection(result)
      } catch {
        // silent — door detection is best-effort
      }
    },
    [setDoorDetection],
  )

  // Paste handler - uses extractDominantColor and detectDoorSilently defined above
  useEffect(() => {
    const onPaste = (event: ClipboardEvent) => {
      ...
      if (!storefrontPreviewUrl) {
        ...
        setStorefront(file, url)
        detectDoorSilently(url)
        return
      }
