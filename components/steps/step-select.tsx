...
import { Download, CheckCircle2, Ruler, ExternalLink } from "lucide-react"
import { useState } from "react"

const MAIN_SITE_URL = process.env.NEXT_PUBLIC_MAIN_SITE_URL ?? "https://kaykovmedia.com"

export function StepSelect() {
  const { generationResult, signSize, doorDetection } = useFlowStore()
    ...
    <div className="space-y-8">
      ...
      {signSize && (
        <div className="flex items-start gap-3 bg-gray-50 border border-gray-200 rounded-2xl p-5">
          <Ruler size={20} className="text-gray-500 mt-0.5 shrink-0" />
          <div>
            <p className="text-sm font-semibold text-gray-900">Estimated sign size</p>
            <p className="text-2xl font-bold text-gray-900 mt-1">{signSize.label}</p>
            <p className="text-xs text-gray-400 mt-1">
              {signSize.method === "door-reference"
                ? `Based on door reference (${Math.round((doorDetection?.confidence ?? 0) * 100)}% confidence)`
                : signSize.method === "user-input"
                ? "Based on your storefront width"
                : "AI estimate — confirm with your sign specialist"}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-black text-white rounded-2xl p-6 text-center space-y-3">
        <p className="text-lg font-bold">Your design is ready.</p>
        <p className="text-sm text-white/70">
          A Kaykov Signage specialist will review your mockup and follow up shortly.
        </p>
        <a
          href={MAIN_SITE_URL}
          className="inline-flex items-center gap-2 mt-2 bg-white text-black text-sm font-semibold px-5 py-2.5 rounded-xl hover:bg-gray-100 transition-colors"
        >
          Back to Kaykov Signage
